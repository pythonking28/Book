  ...
  const submit = async (e) => {
    ...
    await fetch("/api/order/new", {
      ...
      body: JSON.stringify({
        ...
        add_order_query: `CALL add_order("${order_id}", "${customer_id}", ${new Date().getTime()}, "in progress")`,
    ...
    router.push(`/order_status?o_id=${order_id}`);
  };
  const cancel = () => {
        ...
        {!submissionOnProgress && (
          <form method="POST" name="checkout" id="checkout" onSubmit={submit}>
            ...
            <label>
              Phone Number{" "}
              <PhoneEnabledIcon
                sx={{ position: "relative", left: 10, top: 6 }}
              />
            </label>
            <input
              type="tel"
